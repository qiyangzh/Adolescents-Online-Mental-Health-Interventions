...
terms <- c("Directionality","Longduration.c",
           "Passive.c", "Smallsample.c", "Self.guided",
           "CBT", "Elementary.c", "Clinical.population")
#terms <- c("Modality")
formula <- reformulate(termlabels = c(terms))
View(Negative[c("Study","Effect.Size", "Directionality","Longduration.c",
                "Passive.c", "Smallsample.c", "Self.guided",
                "CBT.c", "Elementary.c", "Clinical.population.c")])
...
by_study_outcome <- full %>%
  ...
  dplyr::summarise(
    ...
    `Negative` = first(as.integer(`Negative.Outcomes`)),
...
# x axis
########
# Make sure the negative flag is numeric/integer 0/1
by_study_outcome <- by_study_outcome %>%
  mutate(Negative = as.integer(Negative))  # replace Negative with your column, e.g., `negative outcomes`

# Build Outcomes order: 1s left, 0s right; tie-break alphabetically
outcome_levels <- by_study_outcome %>%
  distinct(Outcomes, Negative) %>%
  arrange(desc(Negative), Outcomes) %>%   # Negative==1 first (left), then 0
  pull(Outcomes)%>%
  unique()       

# Apply the order to the x-axis variable
by_study_outcome <- by_study_outcome %>%
  mutate(Outcomes = factor(Outcomes, levels = outcome_levels))
